...
import {
  decorateOPENAEONProfile,
  ensureProfileCleanExit,
  isProfileDecorated,
} from "./chrome.profile-decoration.js";
import type { ResolvedBrowserConfig, ResolvedBrowserProfile } from "./config.js";
import {
  DEFAULT_OPENAEON_BROWSER_COLOR,
  DEFAULT_OPENAEON_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
export function resolveOPENAEONUserDataDir(profileName = DEFAULT_OPENAEON_BROWSER_PROFILE_NAME) {
...
export async function launchOPENAEONChrome(
  ...
  const userDataDir = resolveOPENAEONUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_OPENAEON_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateOPENAEONProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 openaeon browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`openaeon browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`openaeon browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 openaeon browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopOPENAEONChrome(running: RunningChrome, timeoutMs = 2500) {
