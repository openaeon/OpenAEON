...
const RELAY_TOKEN_CONTEXT = "openaeon-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const OPENAEON_RELAY_BROWSER = "OPENAEON/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.OPENAEON_GATEWAY_TOKEN?.trim() || process.env.AEONPROPHET_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or OPENAEON_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedOPENAEONRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === OPENAEON_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
