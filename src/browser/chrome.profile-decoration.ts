...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openaeon-profile-decorated");
}
 ...
 */
export function decorateOPENAEONProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENAEON_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENAEON_BROWSER_COLOR).toUpperCase();
