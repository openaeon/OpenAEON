...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "openaeon.mjs", entryBasename: "entry.js" },
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.OPENAEON_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.OPENAEON_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.OPENAEON_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.OPENAEON_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[openaeon] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    import("./version.js")
      ...
      .catch((error) => {
        console.error(
          "[openaeon] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[openaeon] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[openaeon] ${parsed.error}`);
      process.exit(2);
    }
