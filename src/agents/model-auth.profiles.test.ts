...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    try {
      ...
      await withEnvAsync(
        {
          OPENAEON_STATE_DIR: tempDir,
          OPENAEON_AGENT_DIR: agentDir,
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.OPENAEON_AGENT_DIR, {
          ...
          const apiKey = await getApiKeyForModel({
            ...
            store,
            agentDir: process.env.OPENAEON_AGENT_DIR,
          });
