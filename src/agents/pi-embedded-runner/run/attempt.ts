...
import { resolveOPENAEONAgentDir } from "../../agent-paths.js";
...
import { resolveOPENAEONDocsPath } from "../../docs-path.js";
...
import { createOPENAEONCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveOPENAEONAgentDir();
    ...
    const docsPath = await resolveOPENAEONDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
            ...
            if (consecutiveErrors >= 3 || chaosScore >= 5) {
              const reason =
                consecutiveErrors >= 3
                  ...
                  : `critical algorithmic divergence (chaosScore=${chaosScore})`;
              log.warn(
              ...
              activeSession
                .steer(
                  `System Hook: Critical Divergence detected (${reason}). You are now in Reflection Mode (Z²). ` +
                    "You are FORBIDDEN from calling any tools in this turn. " +
                    "Analyze the failure, identify the core divergence, and output a detailed `<reflection>` block before proposing a new approach (C).",
                )
                .catch((err) => {
              ...
              activeSession
                .steer(
                  "System Notice: Cognitive Shielding activated. You are entering an Audit Phase due to rising chaos levels. " +
                    "Please double-check your current path, verify tool parameters, and ensure you are not in a logic loop. " +
                    "Consider distilling current verified truths into [AXIOM]s if progress is clear, or rethink your sub-task strategy.",
                )
                .catch((err) => {
                  log.error(`failed to inject audit steering: ${String(err)}`);
                });
            }
          }
        },
      ...
      const queueHandle: EmbeddedPiQueueHandle = {
        ...
        getChaosScore: () => subscription.getChaosScore(),
        getIterationDepth: () => (subscription as any).getIterationDepth?.() ?? 0,
        abort: abortRun,
      };
      ...
      try {
        ...
        // double-compaction. See: https://github.com/openaeon/openaeon/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("openaeon:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/openaeon/openaeon/issues/8643
