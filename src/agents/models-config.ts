...
import { type OPENAEONConfig, loadConfig } from "../config/config.js";
...
export async function ensureOPENAEONModelsJson(
  config?: OPENAEONConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveOPENAEONAgentDir();
