...
import {
  ...
  normalizeStringList,
  parseOPENAEONManifestInstallBase,
  parseFrontmatterBool,
  resolveOPENAEONManifestBlock,
  resolveOPENAEONManifestInstall,
  resolveOPENAEONManifestOs,
  resolveOPENAEONManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  OPENAEONSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseOPENAEONManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveOPENAEONMetadata(
  frontmatter: ParsedSkillFrontmatter,
): OPENAEONSkillMetadata | undefined {
  const metadataObj = resolveOPENAEONManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveOPENAEONManifestRequires(metadataObj);
  const install = resolveOPENAEONManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveOPENAEONManifestOs(metadataObj);
  return {
