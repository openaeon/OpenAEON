...
function resolveBrowserBaseUrl(params: {
  ...
  if (!resolved.enabled) {
    throw new Error(
      "Browser control is disabled. Set browser.enabled=true in ~/.openaeon/openaeon.json.",
    );
  }
  return undefined;
}
...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via OPENAEON's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      'Profiles: use profile="openaeon" for the isolated openaeon-managed standalone browser (default). Use profile="chrome" for Chrome extension relay takeover (your existing Chrome tabs).',
      'If the user mentions the Chrome extension / Browser Relay / toolbar button / "attach tab", ALWAYS use profile="chrome" (do not ask which profile).',
      'When a node-hosted browser proxy is available, the tool may auto-route to it. Pin a node with node=<id|name> or target="node".',
      "Chrome extension relay needs an attached tab: user must click the OPENAEON Browser Relay toolbar icon on the tab (badge ON). If no tab is connected, ask them to attach it.",
    ...
    execute: async (_toolCallId, args) => {
      ...
      const withProfileFailover = async <T>(
        ...
        const p = requestedProfile || "openaeon";
        try {
          return await fn(p);
        } catch (err) {
          const msg = String(err);
          if (p === "chrome" && (msg.includes("no tab is connected") || msg.includes("404:"))) {
            // Attempt auto-failover to openaeon if chrome fails due to no tab or connection
            console.warn(`[BrowserTool] Auto-failover from chrome to openaeon due to: ${msg}`);
          ...
          return await withProfileFailover(profile, async (p) => {
            try {
              ...
              if (msg.includes("404:") && msg.includes("tab not found") && p === "chrome") {
                ...
                if (!tabs.length) {
                  throw new Error(
                    "No Chrome tabs are attached via the OPENAEON Browser Relay extension. Click the toolbar icon on the tab you want to control (badge ON), then retry.",
                    { cause: err },
                  );
                }
                throw new Error(
