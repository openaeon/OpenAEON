...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    previousFastTestEnv = process.env.OPENAEON_TEST_FAST;
  });

  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.OPENAEON_TEST_FAST;
      return;
    }
    process.env.OPENAEON_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    vi.stubEnv("OPENAEON_TEST_FAST", "1");
    agentSpy
      .mockClear()
