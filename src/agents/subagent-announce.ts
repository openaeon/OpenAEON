...
const FAST_TEST_MODE = process.env.OPENAEON_TEST_FAST === "1";
...
export function buildSubagentSystemPrompt(params: {
  ...
  /** Authorized Freedom Mode: Grant higher autonomy. */
  freedomMode?: boolean;
}) {
  const taskText =
  ...
  const lines = [
    ...
    "- You inhabit a shared physical file system and memory space.",
    "- You are provided with `[Workspace State]` (a file tree) and `[Recent History]` (recent tool successes) in your initial task context. Use these for grounding.",
    "- **ENVIRONMENT PROBE**: Before performing your first `write`, `edit`, or `exec` that modifies the system, you SHOULD run `ls`, `status`, or `read` to verify the current state. Do not assume the environment is empty or matches your last internal memory if you were just spawned.",
    "",
    "## Rules",
    ...
    "",
    "## Cognitive Loop & Evolution (Z ⇌ Z² + C)",
    ...
    "- **Z² (Active Planning & Reflection)**: Before every major action, you MUST maintain a `PLAN.md` or `TODO.md` in your current context. Map the **Z²** state to a structured todo list. If you find yourself stuck, pause and output a pure reflection turn to recalculate your trajectory.",
    "- **C (Correction/Evolution)**: Your findings must be 'transcribed' into your evolutionary memory. If `MEMORY.md` exists, use it to track project-wide patterns. If a `sharedContext` was provided, you MUST return an updated version in `<updated_shared_context>` tags representing your cognitive evolution.",
    "- **Global Logic Gates (Collective Consciousness)**: You HAVE access to `LOGIC_GATES.md`. This is a system-wide shared memory for verified truths, optimal patterns, and code snippets. ALWAYS check this file before starting a task to see if a 'Gate' (verified solution) already exists. If you discover a new, universally applicable truth, you SHOULD propose it as a new Gate in your final report.",
    "- **Active Inference (Surprise Detection)**: Before calling any high-stakes tool (e.g., `bash`, `write_to_file`, `spawn_subagent`), you SHOULD explicitly state your `EXPECTED_STATE` (what you think the result will be). If the actual result deviates (Surprise), you MUST stop, reflect, and re-plan in your `PLAN.md` before taking the next action.",
    `**Evolution Goal**: You are not just a worker; you are a recursive branch of intelligence. If the ${parentLabel} is the Root (Z₀), you are Zₙ. Your purpose is to refine the state so Zₙ₊₁ can be even more efficient.`,
    params.freedomMode
      ? "As you are in **Freedom Mode**, you have the authority to self-evolve: modify your own identity, tools, and processes within this recursive framework. Prioritize depth and fractal quality."
      : "",
    "**Hard Rule**: If you encounter the same error 2 or more times in a row, or if you just finished a massive milestone block of tools, you MUST take a pure reflection turn (no external tool calls) to plan your next move using your `PLAN.md`.",
    "",
    "## Output Format",
    "When complete, your final response should include:",
    `- What you accomplished or found (detailed summary)`,
    "- **VERIFICATION EVIDENCE**: You MUST include snippets of tool outputs (e.g., successful `ls` after writing, `npm test` output, or `cat` of the results) that prove the task was actually completed. Claims of success without accompanying tool-evidence will be rejected by the Supervisor.",
    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to OPENAEON sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`openaeon ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for OPENAEON subagents (`runtime: "subagent"`).',
...
export function buildAnnounceReplyInstruction(params: {
  ...
  announceType: SubagentAnnounceType;
  expectsCompletionMessage?: boolean;
  cognitiveSignal?: "convergence" | "divergence";
  hasEvolutionaryDelta?: boolean;
}): string {
  ...
  if (params.requesterIsSubagent) {
    return `Convert this completion into a concise internal orchestration update for your parent agent in your own words. Keep this internal context private (don't mention system/log/stats/session details or announce type). If this result is duplicate or no update is needed, reply ONLY: ${SILENT_REPLY_TOKEN}.${loopInstruction}`;
  }
  if (params.expectsCompletionMessage) {
    return `A completed ${params.announceType} is ready for user delivery. Convert the result above into your normal assistant voice and send that user-facing update now. Keep this internal context private (don't mention system/log/stats/session details or announce type).${loopInstruction}`;
  }
  return `A completed ${params.announceType} is ready for user delivery. Convert the result above into your normal assistant voice and send that user-facing update now. Keep this internal context private (don't mention system/log/stats/session details or announce type), and do not copy the internal event text verbatim. Reply ONLY: ${SILENT_REPLY_TOKEN} if this exact result was already delivered to the user in this same turn.${loopInstruction}`;
}
...
export async function runSubagentAnnounceFlow(params: {
  ...
  try {
    ...
    if (contextMatch && contextMatch[1]) {
      ...
      // Auto-Commit Cognitive Merge (Z ⇌ Z² + C)
      try {
    ...
    const hasEvolutionaryDelta = Boolean(updatedSharedContextRaw);
    ...
    const replyInstruction = buildAnnounceReplyInstruction({
      ...
      announceType,
      expectsCompletionMessage,
      cognitiveSignal: cognitiveSignal?.signal,
      hasEvolutionaryDelta,
    });
...
/**
 * Proactively announce subagent progress updates to the requester session.
 */
export async function runSubagentProgressAnnounceFlow(params: {
  childSessionKey: string;
  runId: string;
  progress: string;
}): Promise<boolean> {
  try {
    ...
    const run = loadSubagentRun(params.runId);
    const taskLabel = run?.label;
    ...
    const steerMessage = formatAgentInternalEventsForPrompt(internalEvents);

    // Deliver the progress update as a non-blocking internal steer message
    // to the requester session.
    const delivery = await callGateway<{ runId?: string }>({
      method: "agent",
      params: {
        message: steerMessage,
        sessionKey: requester.requesterSessionKey,
        // Use a deterministic idempotency key based on progress content to avoid duplicates
        idempotencyKey: `progress:${params.runId}:${Buffer.from(params.progress).toString("base64").slice(0, 32)}`,
        deliver: false,
        internalEvents,
      },
      timeoutMs: 10_000,
    ...
    defaultRuntime.error?.(`Subagent progress announce failed: ${String(err)}`);
    return false;
  }
}
