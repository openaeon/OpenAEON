...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["OPENAEON_GATEWAY_TOKEN", "OPENAEON_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.OPENAEON_GATEWAY_TOKEN = "env-token";
    process.env.OPENAEON_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "OPENAEON_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["OPENAEON_GATEWAY_PASSWORD", "OPENAEON_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
    delete process.env.OPENAEON_GATEWAY_PASSWORD;
    delete process.env.OPENAEON_GATEWAY_TOKEN;
    setGatewayNetworkDefaults(18789);
  });
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.OPENAEON_GATEWAY_PASSWORD = envPassword;
    }
