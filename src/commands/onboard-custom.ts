...
export async function requestOpenAiResponsesVerification(params: {
  baseUrl: string;
  apiKey: string;
  modelId: string;
}): Promise<VerificationResult> {
...
export function applyCustomApiConfig(params: ApplyCustomApiConfigParams): CustomApiResult {
  ...
  const nextModel = {
    ...
    reasoning: params.compatibility === "openai-responses",
  };
  ...
  while (true) {
    ...
    if (!compatibility) {
      ...
      if (openaiProbe.ok) {
        ...
        const responsesProbe = await requestOpenAiResponsesVerification({
          baseUrl,
          apiKey: resolvedApiKey,
          modelId,
        });
        if (responsesProbe.ok) {
          ...
          if (anthropicProbe.ok) {
            ...
            await prompter.note(
              "This endpoint did not respond to OpenAI (Chat or Responses) or Anthropic style requests.",
              "Endpoint detection",
            );
