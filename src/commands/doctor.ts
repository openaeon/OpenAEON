...
import { resolveOPENAEONPackageRoot } from "../infra/openaeon-root.js";
...
import { noteGitMaintenanceHealth } from "./doctor-git-maintenance.js";
import { noteAutoRepairAdvisor } from "./doctor-auto-repair.js";
...
function resolveMode(cfg: OPENAEONConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OPENAEON doctor");

  const root = await resolveOPENAEONPackageRoot({
  ...
  let cfg: OPENAEONConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("openaeon configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openaeon config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openaeon setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  await noteGitMaintenanceHealth({ shouldRepair: prompter.shouldRepair });
  await noteAutoRepairAdvisor({ shouldRepair: prompter.shouldRepair });
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openaeon doctor --fix")}" to apply changes.`);
  }
