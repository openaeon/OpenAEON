...
vi.mock("../plugins/loader.js", () => ({
  loadOPENAEONPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/openaeon-root.js", () => ({
  resolveOPENAEONPackageRoot,
}));
...
beforeEach(() => {
  ...
  resolveOPENAEONPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENAEON_STATE_DIR;
  originalUpdateInProgress = process.env.OPENAEON_UPDATE_IN_PROGRESS;
  process.env.OPENAEON_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openaeon-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENAEON_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENAEON_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENAEON_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
