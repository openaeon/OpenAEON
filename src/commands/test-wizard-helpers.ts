...
export async function setupAuthTestEnv(
  ...
  process.env.OPENAEON_STATE_DIR = stateDir;
  process.env.OPENAEON_AGENT_DIR = agentDir;
...
export function requireOPENAEONAgentDir(): string {
  const agentDir = process.env.OPENAEON_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENAEON_AGENT_DIR not set");
  }
  return agentDir;
}
