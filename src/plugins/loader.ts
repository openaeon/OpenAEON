...
import { discoverOPENAEONPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OPENAEONPluginDefinition;
  register?: OPENAEONPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OPENAEONPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OPENAEONPluginDefinition;
...
export function loadOPENAEONPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOPENAEONPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "openaeon/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "openaeon/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OPENAEONPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as OPENAEONPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
