...
export type RunCronAgentTurnResult = {
   ...
   * messages.  See: https://github.com/openaeon/openaeon/issues/15692
   */
  delivered?: boolean;
  /**
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.OPENAEON_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.openaeon/agents/<agentId>/agent/).
  ...
  const cfgWithAgentDefaults: OPENAEONConfig = {
    ...params.cfg,
    ...
    const fallbackResult = await runWithModelFallback({
      ...
      run: (providerOverride, modelOverride) => {
        ...
        if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
          ...
          // the configured timeoutSeconds. See: https://github.com/openaeon/openaeon/issues/29774
