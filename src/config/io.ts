...
import type { OPENAEONConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareOPENAEONVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "OPENAEON_GATEWAY_TOKEN",
  "OPENAEON_GATEWAY_PASSWORD",
];
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  openaeon config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  openaeon config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: OPENAEONConfig,
  pathSegments: string[],
): { changed: boolean; next: OPENAEONConfig } {
...
function coerceConfig(value: unknown): OPENAEONConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OPENAEONConfig;
}
...
function stampConfigVersion(cfg: OPENAEONConfig): OPENAEONConfig {
...
function warnIfConfigFromFuture(cfg: OPENAEONConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOPENAEONVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OPENAEON (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OPENAEONConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OPENAEONConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OPENAEONConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OPENAEONConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OPENAEONConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPENAEON_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.OPENAEON_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OPENAEON_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OPENAEON_WATCH_SESSION === "string" &&
        deps.env.OPENAEON_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENAEON_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENAEON_WATCH_COMMAND === "string" &&
        deps.env.OPENAEON_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENAEON_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OPENAEON_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: OPENAEONConfig | null = null;
let runtimeConfigSourceSnapshot: OPENAEONConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENAEON_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENAEON_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): OPENAEONConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): OPENAEONConfig {
