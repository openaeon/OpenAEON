...
function isWhatsAppConfigured(cfg: OPENAEONConfig): boolean {
...
function isGenericChannelConfigured(cfg: OPENAEONConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: OPENAEONConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OPENAEONConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: OPENAEONConfig): string[] {
...
function isPluginDenied(cfg: OPENAEONConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: OPENAEONConfig, pluginId: string): OPENAEONConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
