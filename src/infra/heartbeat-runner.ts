...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OPENAEONConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OPENAEONConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: OPENAEONConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: OPENAEONConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OPENAEONConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OPENAEONConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
type HeartbeatPreflight = HeartbeatReasonFlags & {
  ...
  activeTaskPlan?: string;
  skipReason?: HeartbeatSkipReason;
};

function resolveHeartbeatReasonFlags(reasons?: string[]): HeartbeatReasonFlags {
  const kinds = (reasons ?? []).map((r) => resolveHeartbeatReasonKind(r));
  return {
    isExecEventReason: kinds.includes("exec-event"),
    isCronEventReason: kinds.includes("cron"),
    isWakeReason: kinds.includes("wake") || kinds.includes("hook"),
    isEvolutionReason: (reasons ?? []).some((r) => r === "dreaming" || r === "evolution"),
  };
}
...
async function resolveHeartbeatPreflight(params: {
  ...
  try {
    ...
    if (plan.phase !== "complete") {
      const todoLines = (plan.todos || [])
        .filter((t: any) => t.status !== "done")
        .map((t: any) => `- [ ] ${t.title}`);
      if (todoLines.length > 0) {
        activeTaskPlan = `[ACTIVE TASK CONTINUITY]\nYou have an ongoing task: "${plan.description}" (Phase: ${plan.phase})\nRemaining TODOs:\n${todoLines.join("\n")}\n\nMaintain momentum and drive towards convergence.`;
      }
    }
  } catch {
    // No plan or error reading plan; ignore.
  }
...
function resolveHeartbeatRunPrompt(params: {
  ...
  const isEvolution = params.preflight.isEvolutionReason;
  ...
  if (isEvolution) {
    ...
    if (prompt === basePrompt) {
      ...
      prompt = `[MERGED HEARTBEAT: PEANO SCAN]\n${prompt}\n\n${evolutionNotice}`;
    }
  }

  // Task Continuity: If there's an active task plan, ensure it's visible.
  if (params.preflight.activeTaskPlan) {
    if (prompt === basePrompt || isEvolution) {
      prompt = `${prompt}\n\n${params.preflight.activeTaskPlan}`;
    } else {
      // If prompt was already changed by exec/cron, prepend continuity notice
      prompt = `${params.preflight.activeTaskPlan}\n\n${prompt}`;
    }
  }
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OPENAEONConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
