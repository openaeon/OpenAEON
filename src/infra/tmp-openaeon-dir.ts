...
export const POSIX_OPENAEON_TMP_DIR = "/tmp/openaeon";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredOPENAEONTmpDirOptions = {
...
export function resolvePreferredOPENAEONTmpDir(
  options: ResolvePreferredOPENAEONTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "openaeon" : `openaeon-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[openaeon] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback OPENAEON temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback OPENAEON temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_OPENAEON_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_OPENAEON_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_OPENAEON_TMP_DIR)) {
      return POSIX_OPENAEON_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_OPENAEON_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_OPENAEON_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_OPENAEON_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_OPENAEON_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_OPENAEON_TMP_DIR;
