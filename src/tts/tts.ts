...
import { resolvePreferredOPENAEONTmpDir } from "../infra/tmp-openaeon-dir.js";
...
export function resolveTtsConfig(cfg: OPENAEONConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.OPENAEON_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: OPENAEONConfig): string | undefined {
  ...
  for (const provider of providers) {
    ...
    try {
      if (provider === "edge") {
        ...
        const tempRoot = resolvePreferredOPENAEONTmpDir();
