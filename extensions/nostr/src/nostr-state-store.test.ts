...
import type { PluginRuntime } from "openaeon/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENAEON_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openaeon-nostr-"));
  process.env.OPENAEON_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override =
          stateEnv.OPENAEON_STATE_DIR?.trim() || stateEnv.AEONPROPHET_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".openaeon");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENAEON_STATE_DIR = previous;
    }
