...
OpenAEON's WeChat channel plugin, supporting login authorization via QR code scanning.

## Prerequisites

[OpenAEON](https://docs.openaeon.ai/install) must be installed (the `openaeon` CLI needs to be available).

## Quick Install

```bash
npx -y @tencent-weixin/tencent-weixin-cli install
```
...
```bash
openaeon plugins install "@tencent-weixin/tencent-weixin"
```

### 2. Enable the plugin

```bash
openaeon config set plugins.entries.tencent-weixin.enabled true
```

### 3. QR code login

```bash
openaeon channels login --channel tencent-weixin
```
...
```bash
openaeon config set agents.mode per-channel-per-peer
```
