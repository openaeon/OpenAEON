...
function resolveAccountsDir(): string {
  return path.join(resolveStateDir(), "tencent-weixin", "accounts");
}

/**
 * Path to the persistent get_updates_buf file for an account.
 * Stored alongside account data: ~/.openaeon/tencent-weixin/accounts/{accountId}.sync.json
 */
export function getSyncBufFilePath(accountId: string): string {
  return path.join(resolveAccountsDir(), `${accountId}.sync.json`);
}

/** Legacy single-account syncbuf (pre multi-account): `.tencent-weixin-sync/default.json`. */
