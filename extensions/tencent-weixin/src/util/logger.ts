...
/**
 * Plugin logger — writes JSON lines to the main openaeon log file:
 *   /tmp/openaeon/openaeon-YYYY-MM-DD.log
 * Same file and format used by all other channels.
 */

const MAIN_LOG_DIR = path.join("/tmp", "openaeon");
const SUBSYSTEM = "gateway/channels/tencent-weixin";
const RUNTIME = "node";
const RUNTIME_VERSION = process.versions.node;
const HOSTNAME = os.hostname() || "unknown";
const PARENT_NAMES = ["openaeon"];
...
function resolveMinLevel(): number {
  const env = process.env.OPENAEON_LOG_LEVEL?.toUpperCase();
...
function resolveMainLogPath(): string {
  ...
  return path.join(MAIN_LOG_DIR, `openaeon-${dateKey}.log`);
}
