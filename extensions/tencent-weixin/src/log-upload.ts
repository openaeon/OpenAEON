...
function currentDayLogFileName(): string {
  ...
  return `openaeon-${dateKey}.log`;
}

/**
 * Parse --file argument: accepts a short 8-digit date (YYYYMMDD)
 * like "20260316", a full filename like "openaeon-2026-03-16.log",
...
function resolveLogFileName(file: string): string {
  if (/^\d{8}$/.test(file)) {
    ...
    return `openaeon-${yyyy}-${mm}-${dd}.log`;
  }
  if (/^\d{10}$/.test(file)) {
    const yyyy = file.slice(0, 4);
    const mm = file.slice(4, 6);
...
function mainLogDir(): string {
  return path.join("/tmp", "openaeon");
}

function getConfiguredUploadUrl(config: OpenAEONConfig): string | undefined {
  const section = config.channels?.["tencent-weixin"] as { logUploadUrl?: string } | undefined;
  return section?.logUploadUrl;
}

/** Register the `openaeon tencent-weixin logs-upload` CLI subcommand. */
export function registerWeixinCli(params: { program: CliCommand; config: OpenAEONConfig }): void {
  const { program, config } = params;

  const root = program.command("tencent-weixin").description("Weixin channel utilities");
  ...
  root
    ...
    .action(async (options: { url?: string; file?: string }) => {
      ...
      if (!uploadUrl) {
        console.error(
          `[weixin] No upload URL specified. Pass --url or set it with:\n  openaeon config set channels.tencent-weixin.logUploadUrl <url>`,
        );
        process.exit(1);
      }
