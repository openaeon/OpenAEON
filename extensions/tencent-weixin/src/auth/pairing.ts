...
import { withFileLock } from "openaeon/plugin-sdk";
 ...
 * Path: $OPENAEON_OAUTH_DIR || $OPENAEON_STATE_DIR/credentials || ~/.openaeon/credentials
 */
function resolveCredentialsDir(): string {
  const override = process.env.OPENAEON_OAUTH_DIR?.trim();
 ...
 * Path: `<credDir>/tencent-weixin-<accountId>-allowFrom.json`
 */
export function resolveFrameworkAllowFromPath(accountId: string): string {
  const base = safeKey("tencent-weixin");
