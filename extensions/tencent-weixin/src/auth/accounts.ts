...
import { normalizeAccountId } from "openaeon/plugin-sdk";
...
function resolveWeixinStateDir(): string {
  return path.join(resolveStateDir(), "tencent-weixin");
}
...
/**
 * Resolve the openaeon.json config file path.
 * Checks OPENAEON_CONFIG env var, then state dir.
 */
function resolveConfigPath(): string {
  const envPath = process.env.OPENAEON_CONFIG?.trim();
  if (envPath) return envPath;
  return path.join(resolveStateDir(), "openaeon.json");
}

/**
 * Read `routeTag` from openaeon.json (for callers without an `OpenAEONConfig` object).
 * Checks per-account `channels.<id>.accounts[accountId].routeTag` first, then section-level
 * `channels.<id>.routeTag`. Matches `feat_weixin_extension` behavior; channel key is `"tencent-weixin"`.
 */
export function loadConfigRouteTag(accountId?: string): string | undefined {
  try {
    ...
    const section = channels?.["tencent-weixin"] as Record<string, unknown> | undefined;
...
/**
 * No-op stub — config reload is now handled externally via `openaeon gateway restart`.
 */
...
type WeixinAccountConfig = {
  ...
  enabled?: boolean;
  cdnBaseUrl?: string;
  /** Optional SKRouteTag source; read from openaeon.json when `accountId` is passed to `loadConfigRouteTag`. */
...
export function listWeixinAccountIds(_cfg: OpenAEONConfig): string[] {
...
export function resolveWeixinAccount(
  ...
  const section = cfg.channels?.["tencent-weixin"] as WeixinSectionConfig | undefined;
