...
export type ProcessMessageDeps = {
  accountId: string;
  config: import("openaeon/plugin-sdk").OpenAEONConfig;
...
export async function processOneMessage(
  ...
  try {
    ...
    logger.info(
      `debug-check: accountId=${deps.accountId} debug=${String(debug)} hasContextToken=${Boolean(contextToken)} stateDir=${process.env.OPENAEON_STATE_DIR ?? "(unset)"}`,
    );
