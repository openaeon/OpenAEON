import type { ReplyPayload } from "openaeon/plugin-sdk";
import { stripMarkdown } from "openaeon/plugin-sdk";
...
function generateClientId(): string {
  return generateId("tencent-weixin");
}
...
/**
 * Send a voice message downstream using a previously uploaded file.
 * VoiceItem: media (CDN ref), encode_type (7=mp3 usually for TTS).
 * Includes an optional text caption sent as a separate TEXT item first.
 */
export async function sendVoiceMessageWeixin(params: {
  ...
  const { to, text, uploaded, opts } = params;
  if (!opts.contextToken) {
    logger.error(`sendVoiceMessageWeixin: contextToken missing, refusing to send to=${to}`);
    throw new Error("sendVoiceMessageWeixin: contextToken is required");
  }
  ...
  const voiceItem: MessageItem = {
    ...
    voice_item: {
      ...
      // Default to MP3 (7) as most TTS engines output this.
      // If specific silk/amr conversion is added later, this can be dynamic.
      encode_type: 7,
    },
  };
