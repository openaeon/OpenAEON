 ...
 *   audio/*  → uploadVoiceToWeixin        + sendVoiceMessageWeixin
...
export async function sendWeixinMediaFile(params: {
  ...
  const { filePath, to, text, opts, cdnBaseUrl, audioAsVoice } = params;
  ...
  if (mime.startsWith("audio/") || audioAsVoice) {
    logger.info(`[weixin] sendWeixinMediaFile: uploading voice filePath=${filePath} to=${to}`);
