...
import type { ChannelPlugin, OpenAEONConfig } from "openaeon/plugin-sdk";
...
import { createWeixinActionsTool } from "./tools/weixin-actions.js";
...
const MEDIA_OUTBOUND_TEMP_DIR = "/tmp/openaeon/weixin/media/outbound-temp";
...
function resolveLocalPath(mediaUrl: string): string {
  ...
  // Resolve any relative path (./foo, ../foo, .openaeon/foo, foo/bar) against cwd
...
async function sendWeixinOutbound(params: {
  ...
  if (!account.configured) {
    ...
    throw new Error("weixin not configured: please run `openaeon channels login --channel tencent-weixin`");
  }
  ...
  return { channel: "tencent-weixin", messageId: result.messageId };
}
...
export const weixinPlugin: ChannelPlugin<ResolvedWeixinAccount> = {
  ...
  meta: {
    id: "tencent-weixin",
    label: "tencent-weixin",
    selectionLabel: "tencent-weixin (long-poll)",
  ...
  agentTools: () => [createWeixinActionsTool()],
  agentPrompt: {
    messageToolHints: () => [
  ...
  outbound: {
    ...
    sendMedia: async (ctx) => {
      ...
      if (!account.configured) {
        ...
        throw new Error(
          "weixin not configured: please run `openaeon channels login --channel tencent-weixin`",
        );
      }
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      if (!account.configured) {
        ...
        ctx.log?.error?.(
          `[${account.accountId}] weixin not logged in — run: openaeon channels login --channel tencent-weixin`,
        );
