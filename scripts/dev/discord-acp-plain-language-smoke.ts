...
type DriverMode = "token" | "webhook" | "openaeon";
...
function resolveStateDir(): string {
  const override =
    process.env.OPENAEON_STATE_DIR?.trim() || process.env.AEONPROPHET_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.OPENAEON_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".openaeon");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver openaeon] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) OpenAEON spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|openaeon> Driver transport mode (default: token)\n" +
    ...
    "  --openaeon-bin <path>        OpenAEON CLI binary for driver=openaeon (default: openaeon)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  OPENAEON_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  OPENAEON_DISCORD_SMOKE_DRIVER\n" +
    "  OPENAEON_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  OPENAEON_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  OPENAEON_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  OPENAEON_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  OPENAEON_DISCORD_SMOKE_AGENT\n" +
    "  OPENAEON_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  OPENAEON_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  OPENAEON_DISCORD_SMOKE_POLL_MS\n" +
    "  OPENAEON_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  OPENAEON_DISCORD_SMOKE_OPENAEON_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.OPENAEON_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.AEONPROPHET_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.OPENAEON_DISCORD_SMOKE_DRIVER ||
    process.env.AEONPROPHET_DISCORD_SMOKE_DRIVER ||
    "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "openaeon"
        ? "openaeon"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.OPENAEON_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.AEONPROPHET_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.OPENAEON_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.OPENAEON_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.AEONPROPHET_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.OPENAEON_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.OPENAEON_DISCORD_SMOKE_AGENT ||
    process.env.AEONPROPHET_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.OPENAEON_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.AEONPROPHET_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.OPENAEON_DISCORD_SMOKE_INSTRUCTION ||
    process.env.AEONPROPHET_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.OPENAEON_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.OPENAEON_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.OPENAEON_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const openaeonBin =
    resolveArg("--openaeon-bin") || process.env.OPENAEON_DISCORD_SMOKE_OPENAEON_BIN || "openaeon";
...
async function openaeonCliJson<T>(params: { openaeonBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.openaeonBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`openaeon ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await openaeonCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    openaeonBin: params.openaeonBin,
    args: [
      "message",
      "read",
      "--channel",
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `openaeon-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await openaeonCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("openaeon message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from OpenAEON.`,
        diagnostics: {
          bindingCandidates: [
            {
