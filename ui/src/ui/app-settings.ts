...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as OPENAEONApp);
  }
  ...
  if (host.tab === "skills") {
    await loadSkills(host as unknown as OPENAEONApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as OPENAEONApp);
    await loadToolsCatalog(host as unknown as OPENAEONApp);
    await loadConfig(host as unknown as OPENAEONApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as OPENAEONApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as OPENAEONApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as OPENAEONApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as OPENAEONApp, false);
      }
  ...
  if (host.tab === "nodes") {
    ...
    await loadDevices(host as unknown as OPENAEONApp);
    await loadConfig(host as unknown as OPENAEONApp);
    await loadExecApprovals(host as unknown as OPENAEONApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as OPENAEONApp);
    await loadConfig(host as unknown as OPENAEONApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as OPENAEONApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as OPENAEONApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__OPENAEON_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OPENAEONApp, false),
    loadPresence(host as unknown as OPENAEONApp),
    loadSessions(host as unknown as OPENAEONApp),
    loadCronStatus(host as unknown as OPENAEONApp),
    loadDebug(host as unknown as OPENAEONApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OPENAEONApp, true),
    loadConfigSchema(host as unknown as OPENAEONApp),
    loadConfig(host as unknown as OPENAEONApp),
  ]);
}

export async function loadCron(host: SettingsHost) {
  const cronHost = host as unknown as OPENAEONApp;
  await Promise.all([
    loadChannels(host as unknown as OPENAEONApp, false),
