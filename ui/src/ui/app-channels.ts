...
export async function handleWhatsAppStart(host: OPENAEONApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OPENAEONApp) {
...
export async function handleChannelConfigSave(host: OPENAEONApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OPENAEONApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OPENAEONApp): string {
...
function resolveGatewayHttpAuthHeader(host: OPENAEONApp): string | null {
...
function buildGatewayHttpHeaders(host: OPENAEONApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OPENAEONApp) {
...
export function handleNostrProfileToggleAdvanced(host: OPENAEONApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OPENAEONApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OPENAEONApp) {
  const state = host.nostrProfileFormState;
