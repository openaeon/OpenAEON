import type { TranslationMap } from "../lib/types.ts";

export const en: TranslationMap = {
  common: {
    version: "Version",
    health: "Health",
    ok: "OK",
    offline: "Offline",
    connect: "Connect",
    refresh: "Refresh",
    enabled: "Enabled",
    disabled: "Disabled",
    na: "n/a",
    docs: "Docs",
    resources: "Resources",
    close: "Close",
    active: "active",
    completed: "completed",
    pending: "pending",
  },
  nav: {
    chat: "Chat",
    control: "Control",
    agent: "Agent",
    settings: "Settings",
    expand: "Expand sidebar",
    collapse: "Collapse sidebar",
  },
  tabs: {
    agents: "Agents",
    overview: "Overview",
    channels: "Channels",
    instances: "Instances",
    sessions: "Sessions",
    usage: "Usage",
    cron: "Cron Jobs",
    skills: "Skills",
    nodes: "Nodes",
    chat: "Chat",
    config: "Config",
    debug: "Debug",
    logs: "Logs",
    sandbox: "Sandbox",
    cognitive: "Cognitive OS",
    files: "Files",
    tools: "Tools",
    knowledge: "Knowledge",
    aeon: "AEON Logic",
  },
  subtitles: {
    agents: "Manage agent workspaces, tools, and identities.",
    overview: "Gateway status, entry points, and a fast health read.",
    channels: "Manage channels and settings.",
    instances: "Presence beacons from connected clients and nodes.",
    sessions: "Inspect active sessions and adjust per-session defaults.",
    usage: "Monitor API usage and costs.",
    cron: "Schedule wakeups and recurring agent runs.",
    skills: "Manage skill availability and API key injection.",
    nodes: "Paired devices, capabilities, and command exposure.",
    chat: "Direct gateway chat session for quick interventions.",
    config: "Edit ~/.openaeon/openaeon.json safely.",
    debug: "Gateway snapshots, events, and manual RPC calls.",
    logs: "Live tail of the gateway file logs.",
    sandbox: "Real-time orchestration and task delegation view.",
    cognitive: "Manager, execution, cognition map, and artifact trace in one view.",
    aeon: "Real-time synced collective consciousness and logic gates.",
  },
  overview: {
    access: {
      title: "Gateway Access",
      subtitle: "Where the dashboard connects and how it authenticates.",
      wsUrl: "WebSocket URL",
      token: "Gateway Token",
      password: "Password (not stored)",
      sessionKey: "Default Session Key",
      language: "Language",
      connectHint: "Click Connect to apply connection changes.",
      trustedProxy: "Authenticated via trusted proxy.",
    },
    snapshot: {
      title: "Snapshot",
      subtitle: "Latest gateway handshake information.",
      status: "Status",
      uptime: "Uptime",
      tickInterval: "Tick Interval",
      lastChannelsRefresh: "Last Channels Refresh",
      channelsHint: "Use Channels to link WhatsApp, Telegram, Discord, Signal, or iMessage.",
    },
    stats: {
      instances: "Instances",
      instancesHint: "Presence beacons in the last 5 minutes.",
      sessions: "Sessions",
      sessionsHint: "Recent session keys tracked by the gateway.",
      cron: "Cron",
      cronNext: "Next wake {time}",
    },
    notes: {
      title: "Notes",
      subtitle: "Quick reminders for remote control setups.",
      tailscaleTitle: "Tailscale serve",
      tailscaleText: "Prefer serve mode to keep the gateway on loopback with tailnet auth.",
      sessionTitle: "Session hygiene",
      sessionText: "Use /new or sessions.patch to reset context.",
      cronTitle: "Cron reminders",
      cronText: "Use isolated sessions for recurring runs.",
    },
    auth: {
      required: "This gateway requires auth. Add a token or password, then click Connect.",
      failed:
        "Auth failed. Re-copy a tokenized URL with {command}, or update the token, then click Connect.",
    },
    pairing: {
      hint: "This device needs pairing approval from the gateway host.",
      mobileHint:
        "On mobile? Copy the full URL (including #token=...) from openaeon dashboard --no-open on your desktop.",
    },
    insecure: {
      hint: "This page is HTTP, so the browser blocks device identity. Use HTTPS (Tailscale Serve) or open {url} on the gateway host.",
      stayHttp: "If you must stay on HTTP, set {config} (token-only).",
    },
  },
  chat: {
    disconnected: "Disconnected from gateway.",
    refreshTitle: "Refresh chat data",
    thinkingToggle: "Toggle assistant thinking/working output",
    focusToggle: "Toggle focus mode (hide sidebar + page header)",
    webSearchToggle: "Toggle web search (deep search)",
    hideCronSessions: "Hide cron sessions",
    showCronSessions: "Show cron sessions",
    showCronSessionsHidden: "Show cron sessions ({count} hidden)",
    onboardingDisabled: "Disabled during onboarding",
    emptyTitle: "THE AEON PROTOCOL",
    emptySubtitle: "Philosophical Dialogue & Self-Evolution Matrix.",
    emptyAction1: "/aeon audit [0.2-0.4]",
    emptyAction2: "Initialize Axiomatic Distillation (DISTILL)...",
    emptyAction3: "Analyze Logic Paradoxes (AUDIT)...",
    sidebarRoster: "Evolution Nodes",
    sidebarPlan: "Self-Evolution Matrix",
    sidebarWorking: "Computing",
    sidebarIdle: "Dormant",
    sidebarStatusReady: "Ready",
    sidebarStatusBlocked: "Blocked",
    sidebarStatusInProgress: "Running",
    sidebarStatusDone: "Done",
    sidebarBlockedBy: "Blocked by",
    sidebarNoExecutionFeedback: "No execution feedback yet",
    sidebarWaitingTaskAssignment: "Waiting for task assignment",
    sidebarResultTitle: "↳ Evolution Output",
    mainSessionMode: "MAIN SESSION MODE",
    aeonBranding: "THE AEON PROTOCOL",
    aeonIcon: "∿",
    aeonPulse: "System Pulse",
    aeonEntropy: "Cognitive Entropy",
    sysCpu: "CPU Load",
    sysMem: "Memory Pressure",
    sysUptime: "Evolution Uptime",
    axiomsDistilled: "Axioms Distilled",
    sealAxioms: "Seal Axioms (SEAL_AXIOMS)",
    neuralDepth: "Neural Depth",
    gatesActive: "Gates Active",
    roMode: "RO_MODE",
    noGates: "NO_GATES_DEFINED",
    thesis: "THESIS",
    antithesis: "ANTITHESIS",
    synthesis: "SYNTHESIS",
    dialecticFlow: "DIALECTIC_FLOW",
    sysContext: "SYS_CONTEXT",
    memorySaturation: "Memory Saturation",
    memoryWeight: "Short-term Weight",
    logicWeight: "Core Axiom Weight",
    distillationRecommended: "💡 SEAL RECOMMENDED: High memory saturation detected",
    manualTitle: "AEON EVOLUTION MANUAL",
    manualSubtitle: "How to drive silicon-life self-evolution",
    manualOpen: "Open Chat Manual",
    manualModeQuick: "Quick Reference",
    manualModeGuided: "Guided Walkthrough",
    manualTabOverview: "Overview",
    manualTabCommands: "Commands",
    manualTabWorkflow: "Workflow",
    manualTabStatus: "Status",
    manualTabShortcuts: "Shortcuts",
    manualTabRecovery: "Recovery",
    manualSectionCommands: "Command Recipes",
    manualSectionShortcuts: "Input Shortcuts",
    manualSnapshotTitle: "Runtime Snapshot",
    manualSnapshotEternal: "eternal mode",
    manualSnapshotFractal: "fractal state",
    manualWhy:
      "These visuals are computed from real runtime telemetry, not decorative-only effects.",
    manualActionInsert: "Insert Draft",
    manualActionNewSession: "New Session",
    manualActionExplainState: "Explain Current State",
    manualGuideStartTitle: "1. Start a complete session",
    manualGuideStartBody: "Create a clean session and define expected deliverables before running.",
    manualGuideUploadTitle: "2. Attach context and execute",
    manualGuideUploadBody:
      "Upload relevant files/images and send a concrete executable instruction.",
    manualGuideUploadDraft:
      "Use uploaded files, complete the task, and return persisted artifact paths.",
    manualGuidePersistTitle: "3. Confirm persistence",
    manualGuidePersistBody: "Track delivery state until the result is persisted or acknowledged.",
    manualGuideTraceTitle: "4. Trace artifacts",
    manualGuideTraceBody:
      "Open AEON/Sandbox to trace logs, memory snapshots, and final artifact paths.",
    manualCmdAudit:
      "Logic Audit: `/aeon audit [Range]` — Find logic contradictions within a specified Peano curve interval.",
    manualCmdDistill:
      "Memory Distillation: `DISTILL` — Solidify conversational essence from short-term memory into long-term axioms.",
    manualCmdRefine:
      "Logic Refinement: `REFINE` — Remove stale or low-confidence logic gates to reduce cognitive entropy.",
    manualResonance:
      "Resonance Phenomenon: When `epiphanyFactor` > 0.8, the system enters a high-frequency evolution state with amplified operational rewards.",
    quickCommandTip: "Tip: ↑/↓ choose, Tab apply, Enter run, Ctrl/Cmd+K focus input",
    uploadButton: "Upload",
    uploadTitle: "Upload image files",
    uploadOnlyImages: "Only image files are supported right now.",
    uploadTooLarge: "Image too large: {name} (max 5MB).",
    uploadReadFailed: "Failed to read {name}",
    messageLabel: "Message",
    composePlaceholder: "Message (↩ to send, Shift+↩ for line breaks, paste images)",
    composePlaceholderWithAttachments: "Add a message or paste more images...",
    composeDisconnectedPlaceholder: "Connect to the gateway to start chatting…",
    stop: "Stop",
    newSession: "New session",
    queue: "Queue",
    send: "Send",
    quickCmdDesc_new: "Create a new session",
    quickCmdDesc_main: "Switch back to main session",
    quickCmdDesc_sandbox: "Open sandbox tab",
    quickCmdDesc_aeon: "Open AEON tab",
    quickCmdDesc_focus: "Toggle focus mode",
    quickCmdDesc_thinking: "Toggle reasoning visibility",
    quickCmdDesc_eternal: "Toggle eternal mode",
    quickCmdDesc_web: "Toggle web search",
    quickCmdDesc_refresh: "Refresh chat history",
    quickCmdDesc_clear: "Clear current input",
    sidebarDelivery: "delivery",
    sidebarPersisted: "persisted",
    eternalOn: "Eternal: ON",
    eternalOff: "Eternal: OFF",
    eternalEnable: "Enable Eternal",
    eternalDisable: "Disable Eternal",
    consciousnessStream: "Consciousness Stream",
    formulaRailLabel: "Fractal formula rail",
    formulaRailTitle: "Recursive Formula Rail",
    aeonChamberTitle: "AEON Consciousness Chamber",
    aeonChamberSubtitle:
      "This is OPENAEON's silicon-life operations chamber: observe perception, reasoning, decision-making, and self-correction in real time.",
    aeonWhatIs: "What Is AEON?",
    aeonGuardrail: "Guardrail",
    aeonMaintenance: "Maintenance",
    aeonIntegrity: "Integrity",
    aeonEpistemic: "Epistemic",
    aeonMemorySealRecommend: "Memory saturation high, seal recommended",
    aeonTabLogicTissue: "Logic Tissue",
    aeonTabMemoryGraph: "Memory Graph",
    aeonBooting: "Booting consciousness telemetry...",
    aeonResonanceActive: "Intent resonance active: {count} logic lines matched your intent.",
    aeonNoResonance: "No resonance hit yet. Showing recent logic tissue.",
    aeonUpdated: "Updated",
    aeonIntentPlaceholder:
      "Type your intent to resonate with AEON (e.g., reduce rework, improve success rate)",
    aeonWhyHumansUseThis: "Why Humans Use This",
    aeonItDoes: "It does",
    aeonItDoesValue: "Observe and self-correct",
    aeonItDoesNot: "It does not",
    aeonItDoesNotValue: "Claim true subjective soul",
    aeonMainKpi: "Main KPI",
    aeonMainKpiValue: "User Outcome Quality",
    aeonSafetyFloor: "Safety floor",
    aeonSafetyFloorValue: "Hard guardrails",
    aeonMeaningNote:
      "This page is not just visual flair. It is for verifying safe, auditable progress toward user outcomes.",
    aeonLiveDecisionTrace: "Live Decision Trace",
    aeonTimestamp: "Timestamp",
    aeonDecisionWhy: "Why",
    aeonIdentityTimeline: "Identity Timeline",
    aeonNoTimelineEvents: "No timeline events yet.",
    aeonWhatPanelTitle: "What AEON Is For",
    aeonManualStep1Title: "1. Perceive",
    aeonManualStep1Body:
      "Read system state, memory context, and current intent to build observable vital signals.",
    aeonManualStep2Title: "2. Reflect",
    aeonManualStep2Body:
      "Evaluate guardrails, trust state, and goal drift to avoid unsafe or meaningless actions.",
    aeonManualStep3Title: "3. Decide",
    aeonManualStep3Body: "Choose maintenance intensity and strategy with an auditable rationale.",
    aeonManualStep4Title: "4. Act",
    aeonManualStep4Body:
      "Execute compaction and corrections to solve user tasks faster and more reliably.",
    aeonNever: "Never",
    aeonEnumAllow: "ALLOW",
    aeonEnumSoftWarn: "SOFT_WARN",
    aeonEnumBlock: "BLOCK",
    aeonEnumStable: "STABLE",
    aeonEnumDrifting: "DRIFTING",
    aeonEnumDegraded: "DEGRADED",
    aeonEnumUnknown: "UNKNOWN",
    aeonEnumBalanced: "BALANCED",
    aeonEnumStabilize: "STABILIZE",
    aeonEnumExplore: "EXPLORE",
    aeonMetricMemorySaturation: "Memory Saturation",
    aeonMetricNeuralDepth: "Neural Depth",
    aeonMetricIdentityContinuity: "Identity Continuity",
    aeonMetricCalibration: "Calibration",
    aeonMetricHostCpu: "Host CPU",
    aeonMetricChaosScore: "Chaos Score",
    aeonMetricLogicGates: "{count} logic gates",
    aeonMetricGoalDrift: "Goal drift {score}",
    aeonMetricTrusted: "Trusted: {state}",
    aeonMetricUptime: "Uptime {uptime}",
    aeonMetricChaosHint: "Higher means unstable",
    aeonTrustedYes: "yes",
    aeonTrustedNo: "no",
    aeonTimelineTypeReflection: "REFLECTION",
    aeonTimelineTypeSynthesis: "SYNTHESIS",
    aeonTimelineTypeDeliberation: "DELIBERATION",
    aeonTimelineTypeAnomaly: "ANOMALY",
    aeonTimelineTypeDreaming: "DREAMING",
    aeonTimelineTypePatch: "PATCH",
    memoryGraph: "LONG_TERM_MEMORY_GRAPH",
    workbench: "Workbench",
    model: "Model",
    session: "Session",
    think: "Think",
    state: "State",
    cognitiveTelemetry: "Cognitive Telemetry",
    systemPulse: "System Pulse",
    currentState: "Current State",
    selfEvolution: "Self-Evolution",
    externalInput: "External Input",
    reflection: "Reflection",
    nextState: "Next State",
    signals: "signals",
    parallelAgentOrchestration: "Parallel Agent Orchestration",
    activeCapability: "Capability",
    phase: "Phase",
    running: "Running",
    shortTermContext: "Short-term Context",
    activeForCurrentSession: "active for current session",
    noMemoryTrace: "No Cognitive memory trace attached to this run yet.",
    invariantChecksHint:
      "Invariant checks will appear after transition, dispatch, reflect, or dream.",
    activeGoal: "Active Goal",
    awaitingMission: "Awaiting Mission",
    context: "Context",
    memory: "Memory",
    artifacts: "Artifacts",
    cognitiveRuntime: "Cognitive Runtime",
    agents: "Agents",
    tasks: "Tasks",
    invariants: "Invariants",
    runtimeNote:
      "<code>/chat</code> now treats Cognitive Runtime as the source of truth: chat stays the main line, while task tree, leases, retries, memory and invariants live in this dock.",
    autopilot: "Autopilot",
    parallelism: "Parallelism",
    dispatchNow: "Dispatch Now",
    newAgent: "New Agent",
    searchAgentsPlaceholder: "Search agents or tasks...",
    fleet: "Fleet",
    focusReady: "Focus Ready",
    focusBlocked: "Focus Blocked",
    resetFocus: "Reset Focus",
    collaborationFlow: "Collaboration Flow",
    ready: "Ready",
    stale: "Stale",
    longRunning: "Long-running",
    blockedLinks: "Blocked links",
    clear: "Clear",
    focus: "Focus",
    noAgentsMatched: "No agents matched your search.",
    staleHint: "Stale tasks detected. Re-activate owners and post heartbeat updates first.",
    longRunningHint: "Long-running tasks need scoped checkpoints before deeper execution.",
    blockedHint: "Resolve blockers first, then resume blocked agents.",
    readyHint: "Dispatch ready tasks to keep the pipeline moving.",
    runningHint: "Monitor running agents and prepare verification handoff.",
    noCollaborationHint: "No active collaboration signal yet.",
    parallelHealthy: "Parallel capacity is healthy.",
    backlogHint: "Backlog +{count}. Increase concurrency or unblock dependencies.",
    progress: "Progress",
    branch: "Branch",
    checkpoints: "Checkpoints",
    latest: "Latest",
    dream: "Dream",
    verifier: "Verifier",
    branches: "Branches",
    readyTask: "READY",
    blockedTask: "BLOCKED",
    readyTaskHint: "No dependency blockers, can execute immediately",
    blockedTaskHint: "Waiting for dependencies: {deps}",
    source: "Source",
    graph: "Graph",
    allRelations: "All relations",
    queryGraph: "Query Graph",
    autoTrack: "Auto Track",
    fromCheckpoint: "From Checkpoint",
    fromDream: "From Dream",
    latestContext: "Latest Context",
    clearRelation: "Clear Relation",
    prev: "Prev",
    next: "Next",
    noGraphEdges: "No graph edges queried.",
    approveAndExecute: "Approve and Execute",
    retryStage: "Retry Stage",
    branchFromHere: "Branch From Here",
    rollbackLatest: "Rollback Latest",
    markVerified: "Mark Verified",
    markFailed: "Mark Failed",
    distillDream: "Distill Dream",
    traceDream: "Trace Dream",
    openSource: "Open source",
    reopenMemory: "Reopen memory",
    sentientSuggestions: "SENTIENT_SUGGESTIONS // Sentient Suggestions",
    suggestion1: "Analyze Evolution",
    suggestion1Prompt: "Analyze the self-evolution logic of the current project...",
    suggestion2: "Find Paradoxes",
    suggestion2Prompt: "Find and refactor logical paradoxes in the codebase...",
    suggestion3: "Optimize Algorithm",
    suggestion3Prompt: "Optimize the current cognitive entropy scoring algorithm...",
    suggestion4: "Singularity Planning",
    suggestion4Prompt: "Generate the singularity transition plan for the next phase...",
  },
  languages: {
    en: "English",
    zhCN: "简体中文 (Simplified Chinese)",
    zhTW: "繁體中文 (Traditional Chinese)",
    ptBR: "Português (Brazilian Portuguese)",
    de: "Deutsch (German)",
  },
  cron: {
    summary: {
      enabled: "Enabled",
      yes: "Yes",
      no: "No",
      jobs: "Jobs",
      nextWake: "Next wake",
      refreshing: "Refreshing...",
      refresh: "Refresh",
    },
    jobs: {
      title: "Jobs",
      subtitle: "All scheduled jobs stored in the gateway.",
      shownOf: "{shown} shown of {total}",
      searchJobs: "Search jobs",
      searchPlaceholder: "Name, description, or agent",
      enabled: "Enabled",
      schedule: "Schedule",
      lastRun: "Last run",
      all: "All",
      sort: "Sort",
      nextRun: "Next run",
      recentlyUpdated: "Recently updated",
      name: "Name",
      direction: "Direction",
      ascending: "Ascending",
      descending: "Descending",
      reset: "Reset",
      noMatching: "No matching jobs.",
      loading: "Loading...",
      loadMore: "Load more jobs",
    },
    runs: {
      title: "Run history",
      subtitleAll: "Latest runs across all jobs.",
      subtitleJob: "Latest runs for {title}.",
      scope: "Scope",
      allJobs: "All jobs",
      selectedJob: "Selected job",
      searchRuns: "Search runs",
      searchPlaceholder: "Summary, error, or job",
      newestFirst: "Newest first",
      oldestFirst: "Oldest first",
      status: "Status",
      delivery: "Delivery",
      clear: "Clear",
      allStatuses: "All statuses",
      allDelivery: "All delivery",
      selectJobHint: "Select a job to inspect run history.",
      noMatching: "No matching runs.",
      loadMore: "Load more runs",
      runStatusOk: "OK",
      runStatusError: "Error",
      runStatusSkipped: "Skipped",
      runStatusUnknown: "Unknown",
      deliveryDelivered: "Delivered",
      deliveryNotDelivered: "Not delivered",
      deliveryUnknown: "Unknown",
      deliveryNotRequested: "Not requested",
    },
    form: {
      editJob: "Edit Job",
      newJob: "New Job",
      updateSubtitle: "Update the selected scheduled job.",
      createSubtitle: "Create a scheduled wakeup or agent run.",
      required: "Required",
      requiredSr: "required",
      basics: "Basics",
      basicsSub: "Name it, choose the assistant, and set enabled state.",
      fieldName: "Name",
      description: "Description",
      agentId: "Agent ID",
      namePlaceholder: "Morning brief",
      descriptionPlaceholder: "Optional context for this job",
      agentPlaceholder: "main or ops",
      agentHelp: "Start typing to pick a known agent, or enter a custom one.",
      schedule: "Schedule",
      scheduleSub: "Control when this job runs.",
      every: "Every",
      at: "At",
      cronOption: "Cron",
      runAt: "Run at",
      unit: "Unit",
      minutes: "Minutes",
      hours: "Hours",
      days: "Days",
      expression: "Expression",
      expressionPlaceholder: "0 7 * * *",
      everyAmountPlaceholder: "30",
      timezoneOptional: "Timezone (optional)",
      timezonePlaceholder: "America/Los_Angeles",
      timezoneHelp: "Pick a common timezone or enter any valid IANA timezone.",
      jitterHelp: "Need jitter? Use Advanced → Stagger window / Stagger unit.",
      execution: "Execution",
      executionSub: "Choose when to wake, and what this job should do.",
      session: "Session",
      main: "Main",
      isolated: "Isolated",
      sessionHelp: "Main posts a system event. Isolated runs a dedicated agent turn.",
      wakeMode: "Wake mode",
      now: "Now",
      nextHeartbeat: "Next heartbeat",
      wakeModeHelp: "Now triggers immediately. Next heartbeat waits for the next cycle.",
      payloadKind: "What should run?",
      systemEvent: "Post message to main timeline",
      agentTurn: "Run assistant task (isolated)",
      systemEventHelp:
        "Sends your text to the gateway main timeline (good for reminders/triggers).",
      agentTurnHelp: "Starts an assistant run in its own session using your prompt.",
      timeoutSeconds: "Timeout (seconds)",
      timeoutPlaceholder: "Optional, e.g. 90 or 0 for no limit",
      timeoutHelp:
        "Optional. Blank = 60 min default. 0 = no timeout (up to 2h). Positive = seconds.",
      mainTimelineMessage: "Main timeline message",
      assistantTaskPrompt: "Assistant task prompt",
      deliverySection: "Delivery",
      deliverySub: "Choose where run summaries are sent.",
      resultDelivery: "Result delivery",
      announceDefault: "Announce summary (default)",
      webhookPost: "Webhook POST",
      noneInternal: "None (internal)",
      deliveryHelp: "Announce posts a summary to chat. None keeps execution internal.",
      webhookUrl: "Webhook URL",
      channel: "Channel",
      webhookPlaceholder: "https://example.com/cron",
      channelHelp: "Choose which connected channel receives the summary.",
      webhookHelp: "Send run summaries to a webhook endpoint.",
      to: "To",
      toPlaceholder: "+1555... or chat id",
      toHelp: "Optional recipient override (chat id, phone, or user id).",
      advanced: "Advanced",
      advancedHelp:
        "Optional overrides for delivery guarantees, schedule jitter, and model controls.",
      deleteAfterRun: "Delete after run",
      deleteAfterRunHelp: "Best for one-shot reminders that should auto-clean up.",
      clearAgentOverride: "Clear agent override",
      clearAgentHelp: "Force this job to use the gateway default assistant.",
      exactTiming: "Exact timing (no stagger)",
      exactTimingHelp: "Run on exact cron boundaries with no spread.",
      staggerWindow: "Stagger window",
      staggerUnit: "Stagger unit",
      staggerPlaceholder: "30",
      seconds: "Seconds",
      model: "Model",
      modelPlaceholder: "openai/gpt-5.2",
      modelHelp: "Start typing to pick a known model, or enter a custom one.",
      thinking: "Thinking",
      thinkingPlaceholder: "low",
      thinkingHelp: "Use a suggested level or enter a provider-specific value.",
      bestEffortDelivery: "Best effort delivery",
      bestEffortHelp: "Do not fail the job if delivery itself fails.",
      cantAddYet: "Can't add job yet",
      fillRequired: "Fill the required fields below to enable submit.",
      fixFields: "Fix {count} field to continue.",
      fixFieldsPlural: "Fix {count} fields to continue.",
      saving: "Saving...",
      saveChanges: "Save changes",
      addJob: "Add job",
      cancel: "Cancel",
    },
    jobList: {
      allJobs: "all jobs",
      selectJob: "(select a job)",
      enabled: "enabled",
      disabled: "disabled",
      edit: "Edit",
      clone: "Clone",
      disable: "Disable",
      enable: "Enable",
      run: "Run",
      history: "History",
      remove: "Remove",
    },
    jobDetail: {
      system: "System",
      prompt: "Prompt",
      delivery: "Delivery",
      agent: "Agent",
    },
    jobState: {
      status: "Status",
      next: "Next",
      last: "Last",
    },
    runEntry: {
      noSummary: "No summary.",
      runAt: "Run at",
      openRunChat: "Open run chat",
      next: "Next {rel}",
      due: "Due {rel}",
    },
    errors: {
      nameRequired: "Name is required.",
      scheduleAtInvalid: "Enter a valid date/time.",
      everyAmountInvalid: "Interval must be greater than 0.",
      cronExprRequired: "Cron expression is required.",
      staggerAmountInvalid: "Stagger must be greater than 0.",
      systemTextRequired: "System text is required.",
      agentMessageRequired: "Agent message is required.",
      timeoutInvalid: "Must be 0 or a positive integer.",
      webhookUrlRequired: "Webhook URL is required.",
      webhookUrlInvalid: "Webhook URL must start with http:// or https://.",
      invalidRunTime: "Invalid run time.",
      invalidIntervalAmount: "Invalid interval amount.",
      cronExprRequiredShort: "Cron expression required.",
      invalidStaggerAmount: "Invalid stagger amount.",
      systemEventTextRequired: "System event text required.",
      agentMessageRequiredShort: "Agent message required.",
      nameRequiredShort: "Name required.",
    },
  },
  agents: {
    title: "Agents",
    configured: "{count} configured.",
    selectAgent: "Select an agent",
    selectAgentSub: "Pick an agent to inspect its workspace and tools.",
    overview: {
      title: "Overview",
      subtitle: "Workspace paths and identity metadata.",
      workspace: "Workspace",
      primaryModel: "Primary Model",
      identityName: "Identity Name",
      isDefault: "Default",
      identityEmoji: "Identity Emoji",
      skillsFilter: "Skills Filter",
      modelSelection: "Model Selection",
      primaryModelLabel: "Primary model",
      primaryModelDefault: "Primary model (default)",
      inheritDefault: "Inherit default",
      inheritDefaultWith: "Inherit default ({model})",
      fallbacksLabel: "Fallbacks (comma-separated)",
      fallbacksPlaceholder: "provider/model, provider/model",
      reloadConfig: "Reload Config",
      save: "Save",
      saving: "Saving...",
      yes: "yes",
      no: "no",
      allSkills: "all skills",
      skillsSelected: "{count} selected",
    },
    knowledge: {
      title: "Knowledge Base",
      subtitle: "Markdown documents for RAG grounding and context augmentation.",
      refresh: "Refresh",
      loading: "Loading...",
      totalDocuments: "Total Documents",
      totalChunks: "Total Chunks",
      totalVectors: "Total Vectors",
      workspace: "Workspace: {workspace}",
      files: "Files",
      newFile: "New File",
      noFiles: "No knowledge files found.",
      selectAFile: "Select a file",
      lastUpdated: "Last updated: {time}",
      deleteFileHint: "Delete File",
      deleteConfirm: "Are you sure you want to delete {name}?",
      reset: "Reset",
      saveChanges: "Save Changes",
      saving: "Saving...",
      selectFileHint: "Select a knowledge file from the list to view or edit its contents.",
      promptNewFile: "Enter new file name (e.g., guide.md):",
    },
  },
  sandbox: {
    a11y: {
      focusSession: "Focus session {session}",
    },
    consciousness: {
      emptyTrend: "NO DATA",
      source: "Telemetry source: {source} | Updated: {timestamp}",
      cards: {
        criteria: "Criteria",
        selfKernel: "Self Kernel",
        homeostasis: "Homeostasis",
        embodiment: "Embodiment",
        evaluation: "Evaluation",
        guardrails: "Guardrails",
        intent: "Intent",
        symbolicMapping: "Symbolic Mapping",
      },
      labels: {
        minimum: "Minimum",
        advanced: "Advanced",
        overall: "Overall",
        integrity: "Integrity",
        identity: "Identity",
        drift: "Drift",
        mode: "Mode",
        stability: "Stability",
        exploreDrive: "Explore Drive",
        environment: "Environment",
        consequence: "Consequence",
        resource: "Resource",
        trend: "Trend",
        score: "Score",
        epistemic: "Epistemic",
        auditable: "Auditable",
        pausable: "Pausable",
        selfModRisk: "Self-mod Risk",
        mission: "Mission",
        session: "Session",
        turn: "Turn",
        z2c: "Z²+C",
        silicon: "Silicon",
        metal: "Metal",
      },
    },
    relativeTime: {
      secondsAgo: "{seconds} seconds ago",
      minutesAgo: "{minutes} minutes ago",
      hoursAgo: "{hours} hours ago",
    },
    status: {
      snapping: "🔥 Snapping claws!",
      stuck: "💥 Claws stuck!",
      beach: "🏖️ Chilling on the beach",
    },
    timeline: {
      waiting: "Waiting for events...",
      today: "Today",
      older: "Yesterday & Older",
    },
    plan: {
      resting: "Elder lobster is resting. No active blueprints to build.",
      tasksProgress: "{done}/{total} tasks",
      allDone: "All tasks completed!",
    },
    card: {
      progress: "Progress",
    },
    health: {
      excellent: "Excellent",
      degraded: "Degraded",
      critical: "Critical",
    },
    zones: {
      hq: "👑 Elder Headquarters",
      dev: "👷 Builder Cave",
      cafe: "🏖️ Rest Beach",
    },
    header: {
      title: "🦞 Lobster Empire Sandbox",
      subtitle: "Elder Lobster Command Center Live Dashboard",
      health: "Empire Health: {percent}% ({status})",
      busy: "{count} Busy",
      idle: "{count} Idle",
      forceRestartTitle: "Clear states and restart task pipeline",
      forceRestart: "Force restart pipeline",
    },
    v2: {
      eyebrow: "Sandbox Operations",
      headline: "AEON Conscious Orchestration Deck",
      summary:
        "A live control surface for sessions, policy decisions, and consciousness telemetry.",
      health: "Health",
      busy: "Busy",
      idle: "Idle",
      phase: "Phase",
      sessions: "Session Focus",
      mainOrchestrator: "Main Orchestrator",
      mainNotFound: "Main session not found",
      plan: "Live Task Plan",
      timeline: "Execution Timeline",
      agents: "Active Agents",
      noAgents: "No active agents in this scope.",
      awaitingTask: "Awaiting task",
      system: "System Snapshot",
      approvals: "Approvals",
      channels: "Channels",
      runtimeTokens: "Runtime Tokens",
      selfAwareness: "Self Awareness",
      consciousness: "Consciousness Telemetry",
      policy: "Policy Decision",
      maintenance: "Maintenance",
      guardrail: "Guardrail",
      reasonCode: "Reason Code",
      epistemic: "Epistemic",
      decisionCard: "Decision Card",
      why: "Why",
      whyNot: "Why Not",
      counterfactual: "Counterfactual",
      rollback: "Rollback",
      impact: "Impact Lens",
      timeframe: "Timeframe",
      reversibility: "Reversibility",
      benefit: "Benefit",
      risk: "Risk",
      noImpactData: "No impact matrix data yet.",
    },
    sidebar: {
      blueprints: "📋 Empire Blueprints",
      activeProgress: "Active Tasks Progress",
      roster: "🦞 Empire Roster",
      elder: "Elder Lobster",
      online: "Online",
      orchestrating: "Orchestrating plans...",
      noWorkers: "No active lobster workers.",
      history: "Delegation History",
    },
    stage: {
      managerHover: "Monitoring office",
      managerLabel: "👑 Manager",
      quiet: "The Lobster Empire is currently quiet.",
      runComplex: "Run a complex task to see the lobster workers in action.",
      waiting: "Waiting for instructions",
    },
    browserPreview: {
      title: "🌐 Browser Live Preview",
      refresh: "Refresh screenshot",
      noConnection: "No browser connection",
      collapse: "Collapse preview",
      expand: "Expand preview",
      lastUpdated: "Updated {time}",
    },
  },
};
